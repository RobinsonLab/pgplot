/*GRFILEIO -- Fast low-level I/O routines (NeXT - Absoft FORTRAN callable).
 * +
 *
...
int GRCFIL(fd, w_fd)
     int *fd;
     int w_fd;  /* Width of fd argument (appended by Absoft FORTRAN) */
{
  return close(*fd);
}
...
int GRWFIL(fd,   nbytes,   buf,
     ...
     int *fd, *nbytes;
     char *buf;
     int w_fd, w_nbytes, w_buf;  /* Width of arguments */
{
